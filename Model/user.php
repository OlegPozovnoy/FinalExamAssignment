...
class User {
    ...
    try{
        ...
        // if there were any errors - leave
            if (strlen($errstring)> 0)
            {
                ...
                $errstring = '';
                $this->redirect('View/error_page.php');  
                return false;
            }
    ...
    if(strlen(filter_input(INPUT_POST,'password')) < 6){
        $errstring .= "Password must be atleast 6 characters";	}

    if (strcmp(filter_input(INPUT_POST,'password') , filter_input(INPUT_POST,'password2'))<>0)    
        {
            $errstring .="<br/>Passsword and password confirmation dont match";  
        }
    ...
    try {
        ...
        $this->db->signUp($username,$password, $email, $password);
